...
function renderGrid(cat,id){
  $(id).innerHTML=products.filter(p=>p.cat===cat).map(p=>`<article class="product-card" data-id="${p.id}"><div class="product-photo no-photo"><div class="quick-add-slot" data-qa="${p.id}">${quickAddHtml(p)}</div></div><div class="product-name">${p.name}</div><div class="product-latin">${p.latin}</div><div class="product-meta"><span>${p.size}</span><strong class="product-price">${money(p.price)}</strong></div><button type="button" class="product-more" data-more="${p.id}">Подробнее</button></article>`).join('');
}
